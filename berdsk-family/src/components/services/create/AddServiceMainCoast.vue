<template>
  <v-layout row justify-center v-show="this.$store.getters.isAdmin">
    <v-dialog v-model="dialog" persistent max-width="700px">
      ...
      <v-card>
        <v-card-title>
          <span class="headline">Добавить основную услугу</span>
        </v-card-title>
        <v-card-text>
  ...
  export default {
    ...
    methods: {
      onSaveGroup: function () {
        this.dialog = false
        this.$store.dispatch('addServiceMainCoast', {name: this.serviceName, coast: this.coast})
      }
    },
    computed: {
      checkValid: function () {
